...
class Migration_Hapus_Kolom_Update_Trigger extends CI_Migration 
{
	public function up()
	{
		echo PHP_EOL . "--- Migrasi 9 ---" . PHP_EOL;

		$this->db->query('ALTER TABLE paket DROP COLUMN id_pbj');
		echo "Kolom id_pbj dihapus di tabel paket".PHP_EOL;
		...
		$this->db->query('
			...
			BEGIN
				...
				VALUE
				(NEW.id_nh, "Negosiasi dimulai", NOW(), NEW.nominal, NEW.ongkir, NEW.tanggal_pengiriman, NEW.catatan_pembeli);
			END
		');
		...
		$this->db->query('
			...
			BEGIN
				...
				VALUE
				(NEW.id_ns, "Negosiasi dimulai", NOW(), NEW.spesifikasi, NEW.nilai, NEW.catatan_pembeli);
			END
		');
	...
	public function down()
	{
		...
		$this->db->query('ALTER TABLE paket ADD COLUMN id_pbj INT(9) NOT NULL');
		echo "Kolom id_pbj ditambah ke tabel paket".PHP_EOL;
	...
	public function _drop_triggers()
	{
